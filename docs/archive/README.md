...
- **the per-run directory tag** of the private working directory this repo was assembled from
  (`runs_<run-tag>/`, `<run-tag>_status.py` and friends). It was not caught by the original sweep and
  it is the reason the audit now splits a personal value into segments: the workspace name itself
  never appears anywhere in these files — only its middle segment does, embedded in a directory and a
  script prefix. A guard that matches whole strings only misses every derived form of a name, which is
  the same lesson as the WDL lesson in [static-checks.md](../static-checks.md), in a different domain.
...
One row of `CHECKPOINT.md` contradicted that bullet: six sandbox submission ids were still literal.
They are placeholder'd now. The **baseline** step ids in the row below it are deliberately left real —
they point into the public baseline workspace, they are the only way a reader with access can check a
number quoted here, and the bullet above is about my own runs, not that workspace.
