...
an adversarial review (`runs_<run-tag>/adversarial_review.md`) and by recomputing cost from saved
...
1. **Sandbox + inputs frozen.** Workspace `<your-ns>/<your-sandbox>`
   ...
   (06-10) created and Terra-validated (`<run-tag>_freeze.py`, `<run-tag>_configs.py`).
2. **Chain 06→10 ran on branch images** `gatk-sv/sv-pipeline:<branch>--ce8318` +
   ...
   **14,609.6 / 1,929**; step 10 alone **315.2 vs 1,672.8** (`<run-tag>_save_metadata.py` →
   `runs_<run-tag>/metadata/*.json`, every tree printed `(tree complete)`).
...
4. **Trained-parameter payload** (`runs_<run-tag>/outputs/`, sha256 in `MANIFEST.json`):
...
6. **Adversarial review + reconciliation** (`runs_<run-tag>/adversarial_review.md`, and the retraction
   table at the end of `runs_<run-tag>/NOTES.md`). Four of my claims did not survive; two of my own
...
|---|---|---|
| sandbox workspace | `<your-ns>/<your-sandbox>` (id `<sandbox-workspace-id>`) | `.venv/bin/python -W ignore <run-tag>_status.py` |
...
| submissions (all `Done`) | 06 `<submission-id-1>` (retry of failed `<submission-id-1-retry>`), 07 `<submission-id-2>`, 08 `<submission-id-3>`, 09 `<submission-id-4>`, 10 `<submission-id-5>` | `<run-tag>_status.py --costs` |
| baseline step ids that are REAL | step 10 = `86003b19`/wf `e2921cf4`; step 09 = `292f8ecb`/`64ad47ce`; step 08 = `23ffd0c6`/`33b2a2ce`; step 07 = `4b24825e`/`3e574322`; step 06 = `df147230`/`746c675b` | `runs_<run-tag>/metadata/*.json` |
...
| evidence dirs | `runs_<run-tag>/{NOTES.md,outputs,compare_pesr,compare_depth,tables,metadata,submissions,watch_*.log}` — **no `adversarial_review.md` file exists; the review is `NOTES.md:482+` "ADVERSARIAL REVIEW RECONCILIATION"** (corrected by §28.2-1) | `ls runs_<run-tag>/` |
...
cd <workspace>
 ...
 .venv/bin/python -W ignore <run-tag>_status.py                 # verify: all 6 submissions Done
 gcloud compute instances list --project <gcp-project> --filter "name~^gsv-" --format "value(name,status)"
 .venv-profile/bin/python profile_summarize.py runs_<run-tag>/compare_pesr \
 ...
 .venv/bin/python -W ignore <run-tag>_save_metadata.py --per-call --target 10-new --target 10-baseline
 sed -n '/^## §23/,$p' CHECKPOINT.md; sed -n '/ADVERSARIAL REVIEW RECONCILIATION/,$p' runs_<run-tag>/NOTES.md
```
...
|---|---|---|
| `profile_summarize.py runs_<run-tag>/compare_pesr` | concordance 0.9895/0.9905, exact 0.990600, `pct_matched` A→B 0.9351 / B→A 0.9836 | drifts ⇒ tables or rule changed |
...
| step-10 input gates | 17/17 inputs resolve; 7 `+ ".tbi"` sidecars exist | `<run-tag>_fetch_compare.sh` re-resolves |

> **Nothing to push from this workspace.** `<workspace>` is **not a git
> repo** (`git rev-parse` → `fatal: not a git repository`), so `CLAUDE.md`, `**` and
> `runs_<run-tag>/**` are untracked-by-design — do not go looking for a commit that contains them. The
...
* Terra's Cromwell returns **no** `subWorkflowMetadata` / `subWorkflowList` even with
  ...
  true 264/1672.8. `<run-tag>_save_metadata.py` now retries once and prints `(tree complete)` or
...
| file | what it is |
|---|---|
| `<run-tag>_{freeze,configs,status,fetch_compare,save_metadata}.py`/`.sh` | freeze inputs → 5 configs → status/cost → fetch+profile+table → auditable metadata |
| `profile_summarize.py` | frozen aggregation rule (label-derived columns, per-side weights, self-test at 1.0000) |
| `runs_<run-tag>/NOTES.md` | evidence log incl. the full retraction table |
| ~~`runs_<run-tag>/adversarial_review.md`~~ **never landed** (§28.2-1) | the unsoftened adversarial review lives in `runs_<run-tag>/NOTES.md:482+` |
| `runs_<run-tag>/metadata/*.json` | raw Cromwell metadata (recursive, completeness-flagged) — the cost audit |
| `runs_<run-tag>/outputs/` (+`MANIFEST.json`) | genotyped VCFs, 4 param tables, SR diagnostics, ploidy, metrics |
| `runs_<run-tag>/compare_{pesr,depth}/` | paired `gatk-sv-profile` outputs (11 modules each) |
| `runs_<run-tag>/tables/batch_tables.tsv` | 316-row table diff: MATCH 11 / DELTA 31 / STRATEGY 6 / MISSING 268 |
...
|---|---|---|---|
| 6 submissions `Done` | all Done | 5 configs → `Done`, `wf=Succeeded` (the 6th is the failed 06 attempt-1, not a "latest per config" row) | CONFIRMED — `<run-tag>_status.py` |
...
| PESR concordance | 0.9895/0.9905, exact 0.990600, pct_matched .9351/.9836 | identical to the digit | CONFIRMED — `profile_summarize.py runs_<run-tag>/compare_pesr` |
| step-10 param payload | SR `10/86/21.949893`+`.1/.6/1/…0.1`, PE `4/83/21.949893`, RD state0 `0.045452815264742/0.006301943860032662` | verbatim in `runs_<run-tag>/outputs/*_geno_params.tsv` | CONFIRMED (also all 7 MANIFEST sha256 re-hash OK) |
...
| step-10 input gates | 17/17 resolve, 7 `.tbi` | WOM required **17**, config binds **17**, last submission resolved **17**, sidecars **7/13** gs:// inputs | CONFIRMED — new `<run-tag>_check_inputs.py` |
...
1. **`runs_<run-tag>/adversarial_review.md` does not exist.** `find` over the whole workspace returns
   nothing; §27's deliverables table and CLAUDE.md both cite it. The review content is real and is
   in `runs_<run-tag>/NOTES.md:482+` ("2026-09-15 — ADVERSARIAL REVIEW RECONCILIATION", retractions
...
2. **Baseline step-08 job count is 21 attempt-records, not 15** (chain baseline **1,935**, not
   ...
   rounding). Reproduce: `<run-tag>_recompute_cost.py` (offline, over `runs_<run-tag>/metadata/*.json`,
...
|---|---|---|
| `<run-tag>_recompute_cost.py` | chain-cost re-check **offline** from `runs_<run-tag>/metadata/*.json`, completeness-flagged, `--groups` roll-up | NOTES "Auditable cost" table + the 28.2-2 correction |
...
| `<run-tag>_check_inputs.py` | WDL-required (womtool) vs config-bound vs submission-resolved, then `.tbi` sidecar probe; `--step 06..10` | §27 step-10 input gates (17/17, 7/13) |

Promote to standing instructions: **pickup gates for this workspace** =
`git ls-remote` both branches; `<run-tag>_status.py`; `gcloud … --filter "name~^gsv-"`;
`profile_summarize.py /tmp/profile_selftest` (1.0000 or the profile venv/jar moved);
`<run-tag>_recompute_cost.py`; `<run-tag>_check_inputs.py`. All read-only, all < 3 min total.
...
| `runs_<run-tag>/NOTES.md` | 3 new sections: jar-pin closure, A/B failure + #965 mechanism table, the RETRACTED §2 correction |
