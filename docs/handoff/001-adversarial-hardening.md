...
> **Re-measured 2026-09-21T22:20Z**, after two follow-ups to the commit this handoff describes: the
> review that fixed the rerun-step guards, the hardcoded batch row, the adoption `TypeError`, the
> frozen-publish guard, miniwdl-in-the-venv and the `--help` side effects (adding `make flake` +
> `scripts/probe_fixes.py`), and then the import of the pi skill into `.pi/skills/gatk-sv-testkit/`
> with `scripts/check_skill.py` and `scripts/check_docs.py` to keep it and the prose honest. The
> presence of those files *is* the marker; there is no SHA here to trust:
> `syntax` **45** files parsed / 0 failed and `undefmods` **28** (four files added since the row
> above: `terra/wdl_flat.py`, `scripts/probe_fixes.py`, `scripts/check_skill.py`,
> `scripts/check_docs.py`), `flake` **33 files, 0 pyflakes findings** (a new phase; SKIPs, naming the
> install command, when flake8 is not importable), `helpsweep` **28 ok, 0 skipped** on a box whose
> interpreter can import `firecloud`/`numpy`/`pysam`/`WDL` — the 16 skips in the row above are that
> interpreter *missing* those, so the skip count measures the environment and **0 failed** is the
> invariant — `selftest` **20 ok, 0 skipped** locally and **17 ok, 3 skipped** with no gatk-sv clone
> (the CI shape: 14 from the row above + the probe tally + the skill check + the doc check),
> `audit` clean over **80** tracked files.
>
