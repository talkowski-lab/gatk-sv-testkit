...
def probe_nested_bindings() -> str:
    ...
    The defect: `bound, nested = {}, []` followed by `(nested if len(parts) > 2 else bound).update(...)`.
    A list has no `.update`, so the FIRST key shaped `Workflow.Call.input` blew straight out of
    `check_maps()` -- and `check_maps()` is what `preflight()` runs before `create` and `validate`, so
    the guard refused by crashing rather than by reporting, and the
    "... nested-call binding(s) not checked" report that line feeds was UNREACHABLE. Call bindings are
    not hypothetical: upstream's own configs carry 5 in
    `single_sample/GATKSVPipelineSingleSample.json.tmpl` (`...MakeCohortVcf.HERVK_reference`,
    `...RefineComplexVariants.n_per_split`, `...AnnotateVcf.par_bed`) and 80 in the
    `GATKSVPipelineBatch` test fixture, and a `RuntimeAttr?` struct input is dotted sub-fields too.

    The second half of the defect matters more than the crash: keying by leaf via `dict.update` silently
    collapses two distinct call bindings that share a leaf name.
    """
    if not (have("firecloud") and have("WDL")):
        raise Skip("firecloud + WDL (miniwdl)")
    fresh({"GSVTK_PROJECT": "probe", "GSVTK_TERRA_NAMESPACE": "probe-sandbox",
    ...
    install_recorder(sys.modules["terra"])
    out = io.StringIO()
    NESTED = "GenotypeBatch.ProbeCpxRefine.probe_input"

    # Both fixtures are generated BEFORE anything is injected: `_fixture_wdl` derives the "declared" side
    # from CONFIGS, so injecting first would make the fixture declare a bogus required top-level input
    # and the probe would be asserting on its own scaffolding.
    clean, bad = TMP / "wdl-nested-clean", TMP / "wdl-nested-bad"
    _fixture_wdl(clean, None, False)
    _fixture_wdl(bad, "training_vcf", True)

    # CONTROL, direction 1: with no call binding in the map the count must be absent, so the "1 NOT
    # checked" asserted below cannot be a constant the probe prints about itself.
    with contextlib.redirect_stdout(out):
        rc_plain = bc.check_maps(str(clean), only="10-GenotypeBatch")
    if rc_plain != 0 or "NOT checked" in out.getvalue():
        raise AssertionError(f"the no-call-binding control was not clean: {out.getvalue()[-300:]}")

    # CONTROL, direction 2: the comparable findings must be reachable in this fixture at all, established
    # BEFORE the injected key exists -- otherwise a green result here could only mean "it crashed".
    out.truncate(0)
    out.seek(0)
    with contextlib.redirect_stdout(out):
        rc_bad_before = bc.check_maps(str(bad), only="10-GenotypeBatch")
    base = out.getvalue()
    if rc_bad_before == 0 or "EXTRA  GenotypeBatch.training_vcf" not in base \
            or "MISSING GenotypeBatch.probe_required_unbound" not in base:
        raise AssertionError(f"the findings fixture does not produce its findings: {base[-300:]}")

    bc.CONFIGS["10-GenotypeBatch"]["inputs"][NESTED] = "this.probe_nested"

    # The fix: counted, and named with the callee it binds on, without failing a check that has nothing
    # to complain about (a call binding is legitimate wiring).
    out.truncate(0)
    out.seek(0)
    with contextlib.redirect_stdout(out):
        rc = bc.check_maps(str(clean), only="10-GenotypeBatch")
    txt = out.getvalue()
    if "1 nested-call binding(s) NOT checked" not in txt:
        raise AssertionError(f"the call binding is invisible again: {txt[-300:]}")
    if NESTED not in txt or "ProbeCpxRefine" not in txt:
        raise AssertionError(f"counted but never named with its callee: {txt[-300:]}")
    if rc != 0:
        raise AssertionError("reporting a call binding must not fail the check")

    # The point: everything comparable is still comparable. Before the fix the crash happened while
    # building `bound`, so not one finding could print.
    out.truncate(0)
    out.seek(0)
    with contextlib.redirect_stdout(out):
        rc_bad = bc.check_maps(str(bad), only="10-GenotypeBatch")
    txt2 = out.getvalue()
    if rc_bad == 0 or "EXTRA  GenotypeBatch.training_vcf" not in txt2 \
            or "MISSING GenotypeBatch.probe_required_unbound" not in txt2:
        raise AssertionError(f"a call binding suppressed the real findings: {txt2[-400:]}")
    if "1 nested-call binding(s) NOT checked" not in txt2:
        raise AssertionError("the un-compared binding stopped being reported next to the findings")
    if REQUESTS:
        raise AssertionError(f"check_maps made {len(REQUESTS)} request(s); it is an offline check")
    return ("call bindings counted AND named, both controls hold, and EXTRA/MISSING still fire with one "
            "present")
...
PROBES = [
    ...
    ("nested_bindings", probe_nested_bindings),
]
